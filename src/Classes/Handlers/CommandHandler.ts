...
export class CommandHandler {
   ...
   */
  private validateApplicationCommand(
    ApplicationCommand: ContextMenuCommand | ChatInputCommand,
    type: ApplicationCommandType
  ) {
    assert(
      typeof ApplicationCommand.execute !== "undefined",
      "execute function not present"
    );
    assert(
      typeof ApplicationCommand.builder !== "undefined",
      "builder is not present"
    );
    if (ApplicationCommand.type !== type)
      new Error("Command Type does not match Expected");
  }
  ...
  addChatCommands(commands: Collection<string, ChatInputCommand>) {
    for (const [name, command] of commands) {
      try {
        this.validateApplicationCommand(
          command,
        ...
        this.client.emit(
          Events.Error,
          new Error(`Command "${name}" had an error: {error}`)
        );
      }
    }
  ...
  async register() {
    ...
    this.client.emit(
      Events.Debug,
      `Deployed ${sentCommands.size} global command(s)`
    );
